...
while(restartcam == 1)
    ...
    clearvars -except dOTF counter_ counterloop restartcam select scim bg cSb SCIbkgd Datasubfolderbydate nframes dirnamebkgrnddark extratitle filterselection bkgrnddarkload B p2 p1 p3 p7 p8 p9 points
    close all
    closepreview
    clc
srcscim=getselectedsource(scim);
    ...
    switch(select)
        ...
        case 2
            ...
            %         preview(fpm);
                %Choose shutter speed
                disp(' ')
                disp('Science image shuttSCIbkgder speed (1-531)')
            ...
            switch(extratitle)
                case 1
                    ...
                    %make the folder
                    mkdir(['_Batch',num2str(B),'_',filterselection,'_',et]);
                    %store folder name to variable
                    dirnamebatchfolder = ['_Batch',num2str(B),'_',filterselection,'_',et];
                ...
                case 0
                    ...
                    %make the folder
                    mkdir(['_Batch',num2str(B),'_',filterselection]);
                    %store folder name to variable
                    dirnamebatchfolder = ['_Batch',num2str(B),'_',filterselection];
                    %Change into folder that has Previously Created Dark and Background IMages
                    cd(dirnamebkgrnddark)
            ...
            disp(' ')
            disp('Piston Segment 19 by 0.15 micron (lambda / 4)?');
            disp('Yes           [1]')
            disp('No            [0]')
            segfinger  = input(':');
            if segfinger == 1
